...
//Check if in air
if (!collision_rectangle(x+4,y+12,x+12,y+15,block,true,true)) {grounded=0; }
...
if (collision_rectangle(x+4,y+8,x+12,y+16,megaman,true,true) && visible == 1)
{
    if (megaman.paused != 1)
    {
        ...
        {
            if (variable_global_get(CURRENT_WEAPON) < 28)
            {
                visible = 0;
                alarmfill = 1;
...
if (alarmfill > -1) 
{
    ...
    {
        alarmfill = 1;
        if (hlth > 0 && variable_global_get(CURRENT_WEAPON) < 28)
        {
            wpen = variable_global_get(CURRENT_WEAPON)+1;
            variable_global_set(CURRENT_WEAPON,wpen);
            hlth-=1;
        }
        else 
        {
